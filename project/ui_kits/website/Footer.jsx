  ...
  return (
    <footer style={{ background: "var(--color-cloud-gray)", padding: "var(--spacing-64) var(--page-gutter) var(--spacing-40)", fontFamily: "var(--font-dm-sans)", fontFeatureSettings: "var(--font-features)" }}>
      <div style={{ maxWidth: "var(--page-max-width)", margin: "0 auto", display: "flex", flexWrap: "wrap", gap: "var(--spacing-64)" }}>
        ...
        {columns.map((c) => (
          <div key={c[0]} style={{ display: "flex", flexDirection: "column", gap: "var(--spacing-12)", minWidth: 0, flex: "1 1 160px" }}>
            ...
            {c[1].map((l) => (
              <a
                ...
                style={{ fontSize: "var(--text-body-sm)", color: "var(--color-canopy-green)", textDecoration: "none", display: "flex", alignItems: "center", minHeight: "var(--tap-target)" }}
              >
                {l}
              </a>
            ))}
      ...
      <div style={{ maxWidth: "var(--page-max-width)", margin: "var(--spacing-48) auto 0", paddingTop: "var(--spacing-20)", borderTop: "1px solid var(--color-frost-gray)", display: "flex", flexWrap: "wrap", gap: "var(--spacing-16)", justifyContent: "space-between", fontSize: "var(--text-caption)", lineHeight: 1.5, color: "var(--color-slate)" }}>
        ...
        <span style={{ display: "flex", flexWrap: "wrap", gap: "var(--spacing-20)" }}>
          {NAV_LINKS.map((l) => (
            <a key={l} href="#" onClick={(e) => { e.preventDefault(); if (props.onNavigate) props.onNavigate(l); }} style={{ color: "var(--color-slate)", textDecoration: "none", display: "inline-flex", alignItems: "center", minHeight: "var(--tap-target)" }}>{l}</a>
          ))}
        </span>
      </div>
    </footer>
